...
export class CopilotAgent extends MarkdownRulesAgent {
  constructor() {
    const definition: AgentDefinition = {
      ...
      configFiles: [
        ...
        },
        {
          ...
          type: 'file',
          optional: true,
          description: 'GitHub Copilot MCP server configuration',
        },
      ],
  ...
  async applyMCPConfig(projectPath: string, servers: MCPServerConfig[]): Promise<void> {
    ...
    for (const server of servers) {
      ...
      existingConfig.servers[server.name] = nextServer;
    }
  ...
  async removeMCPServer(projectPath: string, serverName: string): Promise<boolean> {
    ...
    try {
      ...
      if (!config.servers || !(serverName in config.servers)) return false;
